...
//
// Created on:  2014/8/8 at 下午 02:16:40 by wang fang.
...
int CheckAuth(OPERATION op)
{
	...
	{
		MessagePopup("自動狀態","自動狀態，不容許執行手動操作！");
		return 0;
	}
...
void initUserPanel (int Panel_Handle, int Control_ID)  
{
	...
	if(GetFileInfo(FileName,&size) == 1)
	{
		...
		while(offset < size)
		{
			...
			InsertTableCellRingItem (Panel_Handle, Control_ID, cell, -1, "機臺操作工");
            InsertTableCellRingItem (Panel_Handle, Control_ID, cell, -1, "設備工程師");
            InsertTableCellRingItem (Panel_Handle, Control_ID, cell, -1, "制程工程師");
            InsertTableCellRingItem (Panel_Handle, Control_ID, cell, -1, "系統管理員");
	...
	switch (event)
	{
		case EVENT_COMMIT:
			if(control == PANEL_USER_CHANGEPWD)
			{
				...
				{
					MessagePopup("輸入錯誤","兩次輸入新密碼不壹致！");
					break;
				}
				else if(strlen(newpwd) > 10)
				{
					MessagePopup("輸入錯誤","密碼太長，請保持10個字符以內！");
					break;
				}
			...
			if(control == PANEL_USER_ADD)
			{
				...
				InsertTableCellRingItem (panel, PANEL_USER_TABLE, cell, -1, "機臺操作工");
            	InsertTableCellRingItem (panel, PANEL_USER_TABLE, cell, -1, "設備工程師");
            	InsertTableCellRingItem (panel, PANEL_USER_TABLE, cell, -1, "制程工程師");
            	InsertTableCellRingItem (panel, PANEL_USER_TABLE, cell, -1, "系統管理員");
