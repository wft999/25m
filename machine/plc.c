...
//
// Created on:  2014/7/3 at ¤U¤È 01:07:52 by wang fang.
...
//		resetBit(sys->hsFillChange); 
	...
	for(int j = 0; j < 12; j++)
	{
		...
		if(testBit(sys->tk[i].hsMake,RB) == 0 )
			resetBit(sys->tk[i].hsMakeStop); 
		
		if(testBit(sys->tk[i].hsMMake,RB) == 0 )
			resetBit(sys->tk[i].hsMMakeStop); 
...
int InitReadFromPLC(void)
{
	...
	for(RTANK_ID i = RTK_01; i<= RTK_06; i++)
	{
		...
		cnv._I16[0] = Q2h.rD[j++]; 
		cnv._I16[1] = Q2h.rD[j++];
		sys->rtk[i].MvPosH = cnv._I32;
		
		cnv._I16[0] = Q2h.rD[j++]; 
		cnv._I16[1] = Q2h.rD[j++];
		sys->rtk[i].MvLock = cnv._I32;
		
		cnv._I16[0] = Q2h.rD[j++]; 
		cnv._I16[1] = Q2h.rD[j++];
		sys->rtk[i].MvPosV = cnv._I32;
	}
	...
	for(RTANK_ID i = RTK_06; i<= RTK_12; i++)
	{
		...
		if(i == RTK_06)
		{
			cnv._I16[0] = Q2h.rD[j++]; 
			cnv._I16[1] = Q2h.rD[j++];
			sys->rtk[i].MvPosH2 = cnv._I32;
		
			cnv._I16[0] = Q2h.rD[j++]; 
			cnv._I16[1] = Q2h.rD[j++];
			sys->rtk[i].MvLock2 = cnv._I32;
		
			cnv._I16[0] = Q2h.rD[j++]; 
			cnv._I16[1] = Q2h.rD[j++];
			sys->rtk[i].MvPosV2 = cnv._I32;
		}
		else
		{
			cnv._I16[0] = Q2h.rD[j++]; 
	...
	while (g_exitThread == 0){
		...
		for(cLop = 0; cLop < ROBOT_NUM; cLop++){
			if(sys->rb[cLop].write2Plc){
				...
				Fmt (SwpSTR, "%s<%i[r16w4p0]%i[r16w4p0]", cmd.rid+1, cmd.type);
				if ( Set_Q_1CF4 (_WW_D, 0x01, 1000 + 2 * cLop, 2, SwpSTR, &mPLC) < 0 )		continue;
					
				unsigned short  rD1,rD2;
				if ( Get_Q_1CF4 (_WR_D, 0x00, 1000+ 2 * cLop, 2, &mPLC) < 0)		continue;
				Scan ( mPLC.DTRN, "%s[i*w4]>%x[b2]", 0, &rD1);
				Scan ( mPLC.DTRN, "%s[i*w4]>%x[b2]", 4, &rD2);
				
				if(rD1 == cmd.rid+1 && rD2 == cmd.type)	
		...
		if  (g_updatePos){
			...
			{
				cnv._U32 = (int)sys->rb[g_updatePosRBid].safeHight;
				Fmt (SwpSTR, "%s[a]<%i[r16w4p0]%i[r16w4p0]", cnv._U16[0],cnv._U16[1]); 
				Fmt (SwpSTR, "%s[a]<%i[r16w4p0]%i[r16w4p0]", cnv._U16[0],cnv._U16[1]);
				
				if ( Set_Q_1CF4 (_WW_D, 0x01, 1100 + 4 * g_updatePosRBid, 4, SwpSTR, &mPLC) < 0 )		continue; 
			}
			else if(g_updatePosType == TANK_POS)
			{
				if(g_updatePosRTid == sys->rb[g_updatePosRBid].firstRid && g_updatePosRBid != RB01)
				{
					cnv._U32 = (int)sys->rtk[g_updatePosRTid].MvPosH2;
					Fmt (SwpSTR, "%s[a]<%i[r16w4p0]%i[r16w4p0]", cnv._U16[0],cnv._U16[1]);
				
					cnv._U32 = (int)sys->rtk[g_updatePosRTid].MvLock2;
					Fmt (SwpSTR, "%s[a]<%i[r16w4p0]%i[r16w4p0]", cnv._U16[0],cnv._U16[1]);
				
					cnv._U32 = (int)sys->rtk[g_updatePosRTid].MvPosV2;
					Fmt (SwpSTR, "%s[a]<%i[r16w4p0]%i[r16w4p0]", cnv._U16[0],cnv._U16[1]);
				}
				else
				{
					cnv._U32 = (int)sys->rtk[g_updatePosRTid].MvPosH;
					Fmt (SwpSTR, "%s[a]<%i[r16w4p0]%i[r16w4p0]", cnv._U16[0],cnv._U16[1]);
				
					cnv._U32 = (int)sys->rtk[g_updatePosRTid].MvLock;
					Fmt (SwpSTR, "%s[a]<%i[r16w4p0]%i[r16w4p0]", cnv._U16[0],cnv._U16[1]);
				
					cnv._U32 = (int)sys->rtk[g_updatePosRTid].MvPosV;
					Fmt (SwpSTR, "%s[a]<%i[r16w4p0]%i[r16w4p0]", cnv._U16[0],cnv._U16[1]);
				}
				
				int offset = 0;
				int rbid = RB01;
				while(rbid < g_updatePosRBid){
					offset += sys->rb[rbid].lastRid - sys->rb[rbid].firstRid + 1;
					rbid++;
				}
				offset += g_updatePosRTid - sys->rb[rbid].firstRid;
				
				if ( Set_Q_1CF4 (_WW_D, 0x01, 1116 + 6 * offset, 6, SwpSTR, &mPLC) < 0 )		continue;
				
			}
			else if(g_updatePosType == CLEAN_POS)
			{
				cnv._U32 = (int)sys->rb[0].cleanPosH;
				Fmt (SwpSTR, "%s[a]<%i[r16w4p0]%i[r16w4p0]", cnv._U16[0],cnv._U16[1]);
				
				cnv._U32 = (int)sys->rb[0].cleanLock;
				Fmt (SwpSTR, "%s[a]<%i[r16w4p0]%i[r16w4p0]", cnv._U16[0],cnv._U16[1]);
				
				cnv._U32 = (int)sys->rb[0].cleanPosV;
				Fmt (SwpSTR, "%s[a]<%i[r16w4p0]%i[r16w4p0]", cnv._U16[0],cnv._U16[1]);
				
				if ( Set_Q_1CF4 (_WW_D, 0x01, 1284, 6, SwpSTR, &mPLC) < 0 )		continue;
			}
			

			g_updatePos = 0;
			
			LogPLCEvent(RBPOS_PLC_EVENT);
		}
